...
import com.surfilter.act.thumbnailator.resizers.Resizers;
...
public class ThumbnailsBuilderTest {
	 ...
	 * <li>The thumbnail dimensions are that which is specified by the size
	 * method.</li>
	 * </ol>
	 */
	@Test
	 ...
	 * <ol>
	 * <li>The thumbnail is successfully created.</li>
	 * <li>The thumbnail dimensions are not the same as what is specified by the
	 * size method, but one that keeps the aspect ratio of the original.</li>
	 * </ol>
	 */
	@Test
	...
	public void scalingModeTwice() throws IOException {
		...
		Thumbnails.of(img).size(200, 200).scalingMode(ScalingMode.PROGRESSIVE_BILINEAR)
				.scalingMode(ScalingMode.PROGRESSIVE_BILINEAR).asBufferedImage();
	}
	...
	public void resizerThenScalingMode() throws IOException {
		...
		Thumbnails.of(img).size(200, 200).resizer(Resizers.PROGRESSIVE).scalingMode(ScalingMode.PROGRESSIVE_BILINEAR)
				.asBufferedImage();
	}
	...
	public void resizerFactoryThenScalingMode() throws IOException {
		...
		Thumbnails.of(img).size(200, 200).resizerFactory(DefaultResizerFactory.getInstance())
				.scalingMode(ScalingMode.PROGRESSIVE_BILINEAR).asBufferedImage();
	}
	...
	public void scalingModeThenResizer() throws IOException {
		...
		Thumbnails.of(img).size(200, 200).scalingMode(ScalingMode.PROGRESSIVE_BILINEAR).resizer(Resizers.PROGRESSIVE)
				.asBufferedImage();
	}
	...
	public void scalingModeThenResizerFactory() throws IOException {
		...
		Thumbnails.of(img).size(200, 200).scalingMode(ScalingMode.PROGRESSIVE_BILINEAR)
				.resizerFactory(DefaultResizerFactory.getInstance()).asBufferedImage();
	}
	...
	public void imageTypeCalledSameType() throws IOException {
		...
		BufferedImage thumbnail = Thumbnails.of(img).size(200, 200).imageType(BufferedImage.TYPE_BYTE_GRAY)
				.asBufferedImage();
	...
	public void imageTypeCalledTwice() throws IOException {
		...
		Thumbnails.of(img).size(200, 200).imageType(BufferedImage.TYPE_BYTE_GRAY)
	...
	public void sourceRegion_Region() throws IOException {
		...
		// when
		Thumbnails.of(img).sourceRegion(new Region(new Coordinate(0, 0), new AbsoluteSize(50, 50))).size(50, 50)
				.toFile(outFile);
	 ...
	 * Test for the {@link Thumbnails.Builder} class where,
	 * <ol>
	 * <li>sourceRegion(Position, int, int) is called with width as
	 * non-positive</li>
	 * </ol>
	 * and the expected outcome is,
	 * <ol>
	 ...
	 * Test for the {@link Thumbnails.Builder} class where,
	 * <ol>
	 * <li>sourceRegion(Position, int, int) is called with height as
	 * non-positive</li>
	 * </ol>
	 * and the expected outcome is,
	 * <ol>
	 ...
	 * Test for the {@link Thumbnails.Builder} class where,
	 * <ol>
	 * <li>sourceRegion(int, int, int, int) is called with width as
	 * non-positive</li>
	 * </ol>
	 * and the expected outcome is,
	 * <ol>
	 ...
	 * Test for the {@link Thumbnails.Builder} class where,
	 * <ol>
	 * <li>sourceRegion(int, int, int, int) is called with height as
	 * non-positive</li>
	 * </ol>
	 * and the expected outcome is,
	 * <ol>
	...
	public void outputFormat_SameAsOriginal_DiffersFromFileExtension() throws IOException {
		...
		String formatName = ImageIO.getImageReaders(ImageIO.createImageInputStream(actualOutFile)).next()
				.getFormatName();
		assertEquals("png", formatName);
	 ...
	 * <ol>
	 * <li>outputFormat specified</li>
	 * <li>outputFormatType is specified, but the outputFormat does not support
	 * compression.</li>
	 * </ol>
	 * and the expected outcome is,
	 * <ol>
	...
	public void outputFormat_Checks_FormatSpecifiedAsOriginal_TypeIsDefaultForFormat() throws IOException {
		...
		// when
		Thumbnails.of(f1).size(50, 50).outputFormat(ThumbnailParameter.ORIGINAL_FORMAT)
	...
	public void resizerThenResizerFactory() throws IOException {
		...
		// when
		Thumbnails.of(img).size(50, 50).resizer(Resizers.PROGRESSIVE)
				.resizerFactory(DefaultResizerFactory.getInstance()).asBufferedImage();
	...
	public void resizerFactoryThenResizer() throws IOException {
		...
		// when
		Thumbnails.of(img).size(50, 50).resizerFactory(DefaultResizerFactory.getInstance())
	...
	public void resizerThenAlphainterpolation() throws IOException {
		...
		// when
		BufferedImage thumbnail = Thumbnails.of(img).size(50, 50).resizer(Resizers.PROGRESSIVE)
				.alphaInterpolation(AlphaInterpolation.SPEED).asBufferedImage();
	...
	public void resizerThenDithering() throws IOException {
		...
		// when
		BufferedImage thumbnail = Thumbnails.of(img).size(50, 50).resizer(Resizers.PROGRESSIVE)
				.dithering(Dithering.DEFAULT).asBufferedImage();
	...
	public void resizerThenAntialiasing() throws IOException {
		...
		// when
		BufferedImage thumbnail = Thumbnails.of(img).size(50, 50).resizer(Resizers.PROGRESSIVE)
				.antialiasing(Antialiasing.DEFAULT).asBufferedImage();
	...
	public void resizerThenRendering() throws IOException {
		...
		// when
		BufferedImage thumbnail = Thumbnails.of(img).size(50, 50).resizer(Resizers.PROGRESSIVE)
				.rendering(Rendering.DEFAULT).asBufferedImage();
	...
	public void resizerFactoryThenDithering() throws IOException {
		...
		// when
		Thumbnails.of(img).size(50, 50).resizerFactory(DefaultResizerFactory.getInstance()).dithering(Dithering.DEFAULT)
				.asBufferedImage();
	...
	public void resizerFactoryThenRendering() throws IOException {
		...
		// when
		Thumbnails.of(img).size(50, 50).resizerFactory(DefaultResizerFactory.getInstance()).rendering(Rendering.DEFAULT)
				.asBufferedImage();
	...
	public void cropWithAspectRatioTrueBefore() throws IOException {
		...
		// when
		BufferedImage thumbnail = Thumbnails.of(img).size(100, 50).keepAspectRatio(true).crop(Positions.CENTER)
				.asBufferedImage();
	...
	public void cropWithAspectRatioTrueAfter() throws IOException {
		...
		// when
		BufferedImage thumbnail = Thumbnails.of(img).crop(Positions.CENTER).size(100, 50).keepAspectRatio(true)
				.asBufferedImage();
	...
	public void cropWithAspectRatioFalseBefore() throws IOException {
		...
		// when
		BufferedImage thumbnail = Thumbnails.of(img).size(100, 50).keepAspectRatio(false).crop(Positions.CENTER)
				.asBufferedImage();
	...
	public void cropWithAspectRatioFalseAfter() throws IOException {
		...
		// when
		BufferedImage thumbnail = Thumbnails.of(img).crop(Positions.CENTER).size(100, 50).keepAspectRatio(false)
				.asBufferedImage();
	...
	public void cropCenterThenWatermarkCenter() throws IOException {
		...
		// when
		BufferedImage thumbnail = Thumbnails.of(img).size(100, 100).crop(Positions.CENTER)
				.watermark(Positions.CENTER, watermark, 1.0f, null).asBufferedImage();
	...
	public void cropCenterThenWatermarkTopLeft() throws IOException {
		...
		// when
		BufferedImage thumbnail = Thumbnails.of(img).size(100, 100).crop(Positions.CENTER)
				.watermark(Positions.TOP_LEFT, watermark, 1.0f, null).asBufferedImage();
	...
	public void cropCenterThenWatermarkTopRight() throws IOException {
		...
		// when
		BufferedImage thumbnail = Thumbnails.of(img).size(100, 100).crop(Positions.CENTER)
				.watermark(Positions.TOP_RIGHT, watermark, 1.0f, null).asBufferedImage();
	...
	public void cropCenterThenWatermarkBottomLeft() throws IOException {
		...
		// when
		BufferedImage thumbnail = Thumbnails.of(img).size(100, 100).crop(Positions.CENTER)
				.watermark(Positions.BOTTOM_LEFT, watermark, 1.0f, null).asBufferedImage();
	...
	public void cropCenterThenWatermarkBottomRight() throws IOException {
		...
		// when
		BufferedImage thumbnail = Thumbnails.of(img).size(100, 100).crop(Positions.CENTER)
				.watermark(Positions.BOTTOM_RIGHT, watermark, 1.0f, null).asBufferedImage();
	...
	public void watermarkExifOrientation1File() throws IOException {
		...
		// when
		BufferedImage thumbnail = Thumbnails.of(imgPath).size(100, 100)
				.watermark(Positions.BOTTOM_RIGHT, watermark, 1.0f, null).asBufferedImage();
	...
	public void watermarkExifOrientation1InputStream() throws IOException {
		...
		// when
		BufferedImage thumbnail = Thumbnails.of(imgIS).size(100, 100)
				.watermark(Positions.BOTTOM_RIGHT, watermark, 1.0f, null).asBufferedImage();
	...
	public void watermarkAndCropExifOrientation1File() throws IOException {
		...
		// when
		BufferedImage thumbnail = Thumbnails.of(imgPath).size(50, 100).crop(Positions.CENTER)
				.watermark(Positions.BOTTOM_RIGHT, watermark, 1.0f, null).asBufferedImage();
	...
	public void watermarkAndCropExifOrientation1InputStream() throws IOException {
		...
		// when
		BufferedImage thumbnail = Thumbnails.of(imgIS).size(50, 100).crop(Positions.CENTER)
				.watermark(Positions.BOTTOM_RIGHT, watermark, 1.0f, null).asBufferedImage();
	...
	public void watermarkAndCropNonCenterExifOrientation1File() throws IOException {
		...
		// when
		BufferedImage thumbnail = Thumbnails.of(imgPath).size(50, 100).crop(Positions.TOP_LEFT)
				.watermark(Positions.BOTTOM_RIGHT, watermark, 1.0f, null).asBufferedImage();
