...
import com.surfilter.act.thumbnailator.filters.Canvas;
import com.surfilter.act.thumbnailator.filters.ImageFilter;
import com.surfilter.act.thumbnailator.filters.Pipeline;
import com.surfilter.act.thumbnailator.filters.Rotation;
import com.surfilter.act.thumbnailator.filters.Watermark;
import com.surfilter.act.thumbnailator.filters.WatermarkText;
import com.surfilter.act.thumbnailator.geometry.AbsoluteSize;
import com.surfilter.act.thumbnailator.geometry.Coordinate;
import com.surfilter.act.thumbnailator.geometry.Position;
import com.surfilter.act.thumbnailator.geometry.Positions;
import com.surfilter.act.thumbnailator.geometry.Region;
import com.surfilter.act.thumbnailator.geometry.Size;
import com.surfilter.act.thumbnailator.name.Rename;
import com.surfilter.act.thumbnailator.resizers.BicubicResizer;
import com.surfilter.act.thumbnailator.resizers.BilinearResizer;
import com.surfilter.act.thumbnailator.resizers.DefaultResizerFactory;
import com.surfilter.act.thumbnailator.resizers.FixedResizerFactory;
import com.surfilter.act.thumbnailator.resizers.ProgressiveBilinearResizer;
import com.surfilter.act.thumbnailator.resizers.Resizer;
import com.surfilter.act.thumbnailator.resizers.ResizerFactory;
import com.surfilter.act.thumbnailator.resizers.configurations.AlphaInterpolation;
import com.surfilter.act.thumbnailator.resizers.configurations.Antialiasing;
import com.surfilter.act.thumbnailator.resizers.configurations.Dithering;
import com.surfilter.act.thumbnailator.resizers.configurations.Rendering;
import com.surfilter.act.thumbnailator.resizers.configurations.ScalingMode;
import com.surfilter.act.thumbnailator.tasks.SourceSinkThumbnailTask;
import com.surfilter.act.thumbnailator.tasks.io.BufferedImageSink;
import com.surfilter.act.thumbnailator.tasks.io.BufferedImageSource;
import com.surfilter.act.thumbnailator.tasks.io.FileImageSink;
import com.surfilter.act.thumbnailator.tasks.io.FileImageSource;
import com.surfilter.act.thumbnailator.tasks.io.ImageSource;
import com.surfilter.act.thumbnailator.tasks.io.InputStreamImageSource;
import com.surfilter.act.thumbnailator.tasks.io.OutputStreamImageSink;
import com.surfilter.act.thumbnailator.tasks.io.URLImageSource;
import com.surfilter.act.thumbnailator.util.ThumbnailatorUtils;
 ...
 * <DL>
 * <DT><B>Usage:</B></DT>
 * <DD>The following example code demonstrates how to use the fluent interface
 ...
 * <a href="http://code.google.com/p/thumbnailator/"> Thumbnailator</a> project
 * page.
 * <p>
 * <h2>Important Implementation Notes</h2> Upon calling one of the
 * {@code Thumbnails.of(...)} methods, <em>in the current implementation</em>,
 * an instance of an inner class of this class is returned. In most cases, the
 * returned instance should not be used by storing it in a local variable, as
 * changes in the internal implementation could break code in the future.
 * <p>
 * As a rule of thumb, <em>always method chain from the {@code Thumbnails.of}
 * all the way until the output method (e.g. {@code toFile},
 * {@code asBufferedImage}, etc.) is called without breaking them down into
 * single statements.</em> See the "Usage" section above for the intended use of
 * the Thumbnailator's fluent interface.
 * <DL>
...
public final class Thumbnails {
	 ...
	 * 
	 * @param files
	 *            File names of image files for which thumbnails are to be
	 *            produced for.
	 * @return Reference to a builder object which is used to specify the
	 *         parameters for creating the thumbnail.
	 ...
	 * 
	 * @param files
	 *            {@link File} objects of image files for which thumbnails are
	 *            to be produced for.
	 * @return Reference to a builder object which is used to specify the
	 *         parameters for creating the thumbnail.
	 * @throws NullPointerException
	 ...
	 * 
	 * @param urls
	 *            {@link URL} objects of image files for which thumbnails are to
	 *            be produced for.
	 * @return Reference to a builder object which is used to specify the
	 *         parameters for creating the thumbnail.
	 * @throws NullPointerException
	 ...
	 * 
	 * @param inputStreams
	 *            {@link InputStream}s which provide the images for which
	 *            thumbnails are to be produced for.
	 * @return Reference to a builder object which is used to specify the
	 *         parameters for creating the thumbnail.
	 * @throws NullPointerException
	 *             If the argument is {@code null}.
	 ...
	 * 
	 * @param images
	 *            {@link BufferedImage}s for which thumbnails are to be produced
	 *            for.
	 * @return Reference to a builder object which is used to specify the
	 *         parameters for creating the thumbnail.
	 * @throws NullPointerException
	 ...
	 * 
	 * @param urls
	 *            URLs of the images for which thumbnails are to be produced.
	 * @return Reference to a builder object which is used to specify the
	 *         parameters for creating the thumbnail.
	 * @throws NullPointerException
	 *             If the argument is {@code null}.
	 ...
	 * 
	 * @param inputStreams
	 *            {@link InputStream}s which provide images for which thumbnails
	 *            are to be produced.
	 * @return Reference to a builder object which is used to specify the
	 *         parameters for creating the thumbnail.
	 * @throws NullPointerException
	 ...
	 * {@link #size(int, int)} and {@link #outputQuality(double)} to set up the
	 * thumbnail generation parameters. (See "Intended Use" below.) The end
	 * result should be code that resembles English.
	 * <p>
	 * In most cases, holding an instance of this class in a local variable,
	 * such as seen in the "Unintended Use" example below, is more verbose and
	 * less future-proof, as changes to this class (which is just an inner class
	 * of the {@link Thumbnails} class) can lead to broken code when attempting
	 ...
	 * {@code to} (e.g. {@link #toFiles(Rename)}) or {@code as} (e.g.
	 * {@link #asBufferedImages()}) is called.
	 * <p>
	...
	public static class Builder<T> {
		...
		private static enum Properties implements Property {
			SIZE("size"), WIDTH("width"), HEIGHT("height"), SCALE("scale"), IMAGE_TYPE("imageType"), SCALING_MODE(
					"scalingMode"), ALPHA_INTERPOLATION("alphaInterpolation"), ANTIALIASING("antialiasing"), DITHERING(
							"dithering"), RENDERING("rendering"), KEEP_ASPECT_RATIO("keepAspectRatio"), OUTPUT_FORMAT(
									"outputFormat"), OUTPUT_FORMAT_TYPE("outputFormatType"), OUTPUT_QUALITY(
											"outputQuality"), RESIZER("resizer"), SOURCE_REGION(
													"sourceRegion"), RESIZER_FACTORY("resizerFactory"), ALLOW_OVERWRITE(
															"allowOverwrite"), CROP("crop"), USE_EXIF_ORIENTATION(
																	"useExifOrientation"),;
		...
		/*
		 * Defines the fields for the builder interface, and assigns the default
		 * values.
		 */
		 ...
		 * cropping if cropping is enabled. If cropping is disabled, then this
		 * field should be left {@code null}.
		 */
		 ...
		 * <p>
		 * For example, to create thumbnails which should fit within a bounding
		 * rectangle of 640 x 480, the following code can be used:
		 * 
		 * <pre>
		 * <code>
		Thumbnails.of(image)
		 ...
		 * </pre>
		 * <p>
		 * In the above code, the thumbnail will preserve the aspect ratio of
		 * the original image. If the thumbnail should be forced to the
		 * specified size, the {@link #forceSize(int, int)} method can be used
		 ...
		 * Sets the size of the thumbnail.
		 * <p>
		 * The thumbnails will be forced to the specified size, therefore, the
		 * aspect ratio of the original image will not be preserved in the
		 * thumbnails. Calling this method will be equivalent to calling the
		 * {@link #size(int, int)} method in conjunction with the
		 * {@link #keepAspectRatio(boolean)} method with the value
		 * {@code false}.
		 * <p>
		 * Once this method is called, calling the {@link #scale(double)} method
		 * will result in an {@link IllegalStateException}.
		 ...
		 * 
		 * @param scale
		 *            The scaling factor to use when creating a thumbnail.
		 *            <p>
		 *            The value must be a {@code double} which is greater than
		 *            {@code 0.0}, and not {@link Double#POSITIVE_INFINITY}.
		 * @return Reference to this object.
		 */
		 ...
		 * 
		 * @param scaleWidth
		 *            The scaling factor to use for the width when creating a
		 *            thumbnail.
		 *            <p>
		 *            The value must be a {@code double} which is greater than
		 *            {@code 0.0}, and not {@link Double#POSITIVE_INFINITY}.
		 * @param scaleHeight
		 *            The scaling factor to use for the height when creating a
		 *            thumbnail.
		 *            <p>
		 *            The value must be a {@code double} which is greater than
		 *            {@code 0.0}, and not {@link Double#POSITIVE_INFINITY}.
		...
		/**
		 * Specifies the source region from which the thumbnail is to be created
		 * from.
		 * <p>
		 * Calling this method multiple times will result in an
		 * {@link IllegalStateException} to be thrown.
		 ...
		 * @return Reference to this object.
		 * @throws NullPointerException
		 *             If the source region object is {@code null}.
		 * @since 0.3.4
		 */
		 ...
		 * @return Reference to this object.
		 * @throws NullPointerException
		 *             If the position and/or size is {@code null}.
		 * @since 0.3.4
		 */
		 ...
		 * 
		 * @param x
		 *            The horizontal-compoennt of the top left-hand corner of
		 *            the source region.
		 * @param y
		 *            The vertical-compoennt of the top left-hand corner of the
		 *            source region.
		 * @param width
		 ...
		 * @return Reference to this object.
		 * @throws IllegalArgumentException
		 *             If the width and/or height is less than or equal to
		 *             {@code 0}.
		 * @since 0.3.4
		 */
		 ...
		 * 
		 * @param region
		 *            A rectangular region which specifies the source region to
		 *            use when creating the thumbnail.
		 * @throws NullPointerException
		 *             If the region is {@code null}.
		 * @since 0.3.4
		 */
		 ...
		 * <p>
		 * Internally, the resizing is performed in two steps. First, the
		 * thumbnail will be sized so that one of the dimensions will be sized
		 * exactly to the dimension specified in the {@code size} method, while
		 * allowing the other dimension to overhang the specified dimension.
		 * Then, the thumbnail will be cropped to the dimensions specified in
		 * the {@code size} method, positioned using the speficied
		 * {@link Position} object.
		 * <p>
		 * Once this method is called, calling the {@link #scale(double)} method
		 * will result in an {@link IllegalStateException}.
		 ...
		 *
		 * @param position
		 *            The position to which the thumbnail should be cropped to.
		 *            For example, if {@link Positions#CENTER} is specified, the
		 *            resulting thumbnail will be made by cropping to the center
		 *            of the image.
		 * @throws NullPointerException
		 *             If the position is {@code null}.
		 * @since 0.4.0
		 */
		 ...
		 *            If {@code true} then existing files will be overwritten if
		 *            specified as a destination. If {@code false}, then the
		 *            existing files will not be altered. For specific behavior,
		 *            please refer to the specific output methods listed above.
		 * 
		 * @since 0.3.7
		 */
		...
		/**
		 * Sets the dithering mode when performing the resizing operation to
		...
		/**
		 * Sets the antialiasing mode when performing the resizing operation to
		 * generate the thumbnail.
		 * <p>
		 * Calling this method to set this parameter is optional.
		 * <p>
		...
		/**
		 * Sets the rendering mode when performing the resizing operation to
		 * generate the thumbnail.
		 * <p>
		 * Calling this method to set this parameter is optional.
		 * <p>
		 ...
		 * <p>
		 * If this method is not called when, by default the aspect ratio of the
		 * original image is preserved for the thumbnail.
		 * <p>
		 ...
		 * 
		 * @param keep
		 *            {@code true} if the thumbnail is to maintain the aspect
		 ...
		 *             If
		 *             <ol>
		 *             <li>the {@link #size(int, int)} has not yet been called
		 *             to specify the size of the thumbnail, or</li>
		 *             <li>the {@link #scale(double)} method has been called,
		 *             or</li>
		 *             <li>the {@link #scale(double, double)} method has been
		 *             called, or</li>
		 *             <li>the {@link #width(int)} and/or {@link #height(int)}
		 *             has been called and not preserving the aspect ratio is
		 *             desired.</li>
		 *             </ol>
		 */
		public Builder<T> keepAspectRatio(boolean keep) {
			if (statusMap.get(Properties.SCALE) == Status.ALREADY_SET) {
				throw new IllegalStateException("Cannot specify whether to "
						+ "keep the aspect ratio if the scaling factor has " + "already been specified.");
			}
			if (statusMap.get(Properties.SIZE) == Status.NOT_READY) {
				throw new IllegalStateException("Cannot specify whether to "
						+ "keep the aspect ratio unless the size parameter has " + "already been specified.");
			}
			if ((statusMap.get(Properties.WIDTH) == Status.ALREADY_SET
					|| statusMap.get(Properties.HEIGHT) == Status.ALREADY_SET) && !keep) {
				throw new IllegalStateException("The aspect ratio must be "
						+ "preserved when the width and/or height parameter " + "has already been specified.");
			}
		...
		/**
		 * Sets the output quality of the compression algorithm used to compress
		 * the thumbnail when it is written to an external destination such as a
		 * file or output stream.
		 * <p>
		 ...
		 * Calling this method to set this parameter is optional.
		 * <p>
		 * Calling this method in conjunction with {@link #asBufferedImage()} or
		 * {@link #asBufferedImages()} will not result in any changes to the
		 * final result.
		 * <p>
		 ...
		 * 
		 * @param quality
		 *            The compression quality to use when writing the thumbnail.
		 * @return Reference to this object.
		 * @throws IllegalArgumentException
		 *             If the argument is less than {@code 0.0f} or is greater
		 *             than {@code 1.0f}.
		 */
		 ...
		 * @return Reference to this object.
		 * @throws IllegalArgumentException
		 *             If the argument is less than {@code 0.0d} or is greater
		 *             than {@code 1.0d}.
		 */
		 ...
		 * <p>
		 * For example, to set the output format to JPEG, the following code can
		 * be used:
		 * 
		 * <pre>
		 * <code>
		 ...
		 * dependents on whether the Java Image I/O API recognizes the string as
		 * a format that it supports for output. (Valid format names can be
		 * obtained by calling the {@link ImageIO#getWriterFormatNames()}
		 * method.)
		 * <p>
		 * Calling this method to set this parameter is optional.
		 * <p>
		 ...
		 * 
		 * @param format
		 *            The compression format to use when writing the thumbnail.
		 * @return Reference to this object.
		 * @throws IllegalArgumentException
		 ...
		 * {@link IllegalStateException} to be thrown.
		 * 
		 * @param useExifOrientation
		 *            {@code true} if the Exif metadata should be used to
		 *            determine the orientation of the thumbnail, {@code false}
		 *            otherwise.
		 * @return Reference to this object.
		 * @since 0.4.3
		 */
		 ...
		 * <p>
		 * If the default type for the compression codec should be used, a value
		 * of {@link ThumbnailParameter#DEFAULT_FORMAT_TYPE} should be used.
		 * <p>
		 * Calling this method to set this parameter is optional.
		 * <p>
		 * Calling this method multiple times will result in an
		 ...
		 * {@link #outputFormat} method is disabled, in order to prevent cases
		 * where the output format type does not exist in the format specified
		 * for the {@code outputFormat} method.
		 *
		 * @param formatType
		 *            The compression format type
		 * @return Reference to this object.
		 * @throws IllegalArgumentException
		 *             If an unsupported format type is specified for the
		 *             current output format type. Or, if the output format has
		 *             not been specified before this method was called.
		 */
		public Builder<T> outputFormatType(String formatType) {
			...
			if (!ThumbnailatorUtils.isSupportedOutputFormatType(outputFormat, formatType)) {
				throw new IllegalArgumentException("Specified format type (" + formatType + ") is not "
						+ " supported for the format: " + outputFormat);
			}

			/*
			 * If the output format type is set, then we'd better make the
			 * output format unchangeable, or else we'd risk having a type that
			 * is not part of the output format.
			 */
		 ...
		 * {@link #watermark(Position, BufferedImage, float)} method, where the
		 * opacity is 50%, and the position is set to center of the thumbnail:
		 * <p>
		 * 
		 * <pre>
		...
		public Builder<T> watermark(BufferedImage image) {
			return watermark(Positions.CENTER, image, 0.5f, null);
		}

		/**
		 * Sets the image and opacity of the watermark to apply on the
		 * thumbnail.
		 * <p>
		 * This method is a convenience method for the
		 * {@link #watermark(Position, BufferedImage, float)} method, where the
		 ...
		 *            where {@code 0.0f} is completely transparent, and
		 *            {@code 1.0f} is completely opaque.
		 * @return Reference to this object.
		 */
		public Builder<T> watermark(BufferedImage image, float opacity) {
			return watermark(Positions.CENTER, image, opacity, null);
		}
		 ...
		 * @return Reference to this object.
		 */
		public Builder<T> watermark(Position position, BufferedImage image, float opacity, Integer degree) {
			filterPipeline.add(new Watermark(position, image, opacity, degree));
			return this;
		}
		 ...
		 * @return Reference to this object.
		 */
		public Builder<T> watermark(Position position, String logoText, float opacity, Integer degree, Font font,
				Color color) {
			filterPipeline.add(new WatermarkText(position, logoText, opacity, degree, font, color));
			return this;
		}
		 ...
		 * <p>
		 * If multiple filters are to be applied, the filters will be applied in
		 * the order that this method is called.
		 * <p>
		 * Calling this method to set this parameter is optional.
		 * 
		 * @param filter
		 ...
		 * 
		 * @throws IllegalStateException
		 *             If the builder is not ready to create thumbnails, due to
		 *             some parameters not being set.
		 */
		...
		/**
		 * Returns a {@link Resizer} which is suitable for the current builder
		 * state.
		 * 
		 * @param mode
		 *            The scaling mode to use to create thumbnails.
		 * @return The {@link Resizer} which is suitable for the specified
		 *         scaling mode and builder state.
		 */
		...
		private void prepareResizerFactory() {
			/*
			 * If the scalingMode has been set, then use scalingMode to obtain a
			 * resizer, else, use the resizer field.
			 */
		 ...
		 * 
		 * @return A {@link ThumbnailParameter} from the current builder state.
		 */
		private ThumbnailParameter makeParam() {
			...
			/*
			 * croppingPosition being non-null means that a crop should take
			 * place.
			 */
			...
			if (Double.isNaN(scaleWidth)) {
				...
				if (width == DIMENSION_NOT_SPECIFIED && height == DIMENSION_NOT_SPECIFIED) {
					throw new IllegalStateException("The width or height must be specified. If this "
							+ "exception is thrown, it is due to a bug in the " + "Thumbnailator library.");
				}
				...
				return new ThumbnailParameter(new Dimension(width, height), sourceRegion, keepAspectRatio, outputFormat,
						outputFormatType, outputQuality, imageTypeToUse, filterPipeline.getFilters(), resizerFactory,
						fitWithinDimenions, useExifOrientation);
			} else {
				// If the scaling factor was specified
				return new ThumbnailParameter(scaleWidth, scaleHeight, sourceRegion, keepAspectRatio, outputFormat,
						outputFormatType, outputQuality, imageTypeToUse, filterPipeline.getFilters(), resizerFactory,
						fitWithinDimenions, useExifOrientation);
			}
		}
		 ...
		 * method is preferred over the {@link #asBufferedImages()} method, as
		 * (1) the processing does not have to complete before the method
		 ...
		 * element.
		 * 
		 * @return An {@link Iterable} which will provide an {@link Iterator}
		 *         which returns thumbnails as {@link BufferedImage}s.
		 */
		public Iterable<BufferedImage> iterableBufferedImages() {
			checkReadiness();
			/*
			 * TODO To get the precise error information, there would have to be
		 ...
		 * {@link BufferedImage}s.
		 * <p>
		 * <h3>Note about performance</h3> If there are many thumbnails
		 * generated at once, it is possible that the Java virtual machine's
		 * heap space will run out and an {@link OutOfMemoryError} could result.
		 * <p>
		 ...
		 * @throws IOException
		 *             If an problem occurred during the reading of the original
		 *             images.
		 */
		 ...
		 *             image.
		 * @throws IllegalArgumentException
		 *             If multiple original images are specified.
		 */
		public BufferedImage asBufferedImage() throws IOException {
			...
			Thumbnailator
					.createThumbnail(new SourceSinkThumbnailTask<T, BufferedImage>(makeParam(), source, destination));
		 ...
		 * {@link List} of {@link File}s to the thumbnails.
		 * <p>
		 * When the destination file exists, and overwriting files has been
		 * disabled by calling the {@link #allowOverwrite(boolean)} method with
		 * {@code false}, then the thumbnail with the destination file already
		 * existing will not be written and the corresponding {@code File}
		 * object will not be included in the {@code List} returned by this
		 * method.
		 * <p>
		 ...
		 * 
		 * @param iterable
		 *            An {@link Iterable} which returns an {@link Iterator}
		 *            which returns file names which should be assigned to each
		 *            thumbnail.
		 * @return A list of {@link File}s of the thumbnails which were created.
		 * @throws IOException
		 *             If a problem occurs while reading the original images or
		 *             writing the thumbnails to files.
		 * @since 0.3.7
		 */
		...
		/**
		 * Creates thumbnails and stores them to files using the {@link Rename}
		 * function to determine the filenames. The thubnail files are returned
		 * as a {@link List}.
		 * <p>
		 * When the destination file exists, and overwriting files has been
		 * disabled by calling the {@link #allowOverwrite(boolean)} method with
		 ...
		 * method.
		 * <p>
		 * To call this method, the thumbnails must have been creates from files
		 * by calling the {@link Thumbnails#of(File...)} method.
		 * 
		 * @param rename
		 *            The rename function which is used to determine the
		 *            filenames of the thumbnail files to write.
		 * @return A list of {@link File}s of the thumbnails which were created.
		 * @throws IOException
		 *             If a problem occurs while reading the original images or
		 *             writing the thumbnails to files.
		 * @throws IllegalStateException
		 *             If the original images are not from files.
		 * @since 0.3.7
		 */
		 ...
		 * method.
		 * <p>
		 * Extra caution should be taken when using this method, as there are no
		 * protections in place to prevent file name collisions resulting from
		 * creating thumbnails from files in separate directories but having the
		 * same name. In such a case, the behavior will be depend on the
		 * behavior of the {@link #allowOverwrite(boolean)} as described in the
		 * previous paragraph.
		 * <p>
		 * To call this method, the thumbnails must have been creates from files
		 * by calling the {@link Thumbnails#of(File...)} method.
		 ...
		 *            The destination directory to which the thumbnails should
		 *            be written to.
		 * @param rename
		 *            The rename function which is used to determine the
		 *            filenames of the thumbnail files to write.
		 ...
		 *             If the original images are not from files.
		 * @throws IllegalArgumentException
		 *             If the destination directory is not a directory.
		 * @since 0.4.7
		 */
		 ...
		 * @throws IOException
		 *             If a problem occurs while reading the original images or
		 *             writing the thumbnails to files. thumbnails to files.
		 * @throws IllegalStateException
		 *             If the original images are not from files.
		 * @since 0.3.7
		 */
		 ...
		 * When the destination file exists, and overwriting files has been
		 * disabled by calling the {@link #allowOverwrite(boolean)} method with
		 * {@code false}, then an {@link IllegalArgumentException} will be
		 * thrown.
		 * <p>
		 * To call this method, the thumbnail must have been created from a
		 * single source.
		 * 
		 * @param outFile
		 *            The file to which the thumbnail is to be written to.
		 * @throws IOException
		 *             If a problem occurs while reading the original images or
		 *             writing the thumbnails to files.
		 * @throws IllegalArgumentException
		 *             If multiple original image files are specified, or if the
		 *             destination file exists, and overwriting files is
		 *             disabled.
		 */
		 ...
		 * 
		 * @param os
		 *            The output stream to which the thumbnail is to be written
		 *            to.
		 * @throws IOException
		 *             If a problem occurs while reading the original images or
		 *             writing the thumbnails.
		 * @throws IllegalArgumentException
		 *             If multiple original image files are specified.
		 * @throws IllegalStateException
		 *             If the output format has not been specified through the
		 *             {@link #outputFormat(String)} method.
		 */
		public void toOutputStream(OutputStream os) throws IOException {
			...
			Thumbnailator
					.createThumbnail(new SourceSinkThumbnailTask<T, OutputStream>(makeParam(), source, destination));
		}
		 ...
		 * @param iterable
		 *            An {@link Iterable} which returns an {@link Iterator}
		 *            which returns the output stream which should be assigned
		 *            to each thumbnail.
		 * @throws IOException
		 *             If a problem occurs while reading the original images or
		 *             writing the thumbnails.
		...
		public void toOutputStreams(Iterable<? extends OutputStream> iterable) throws IOException {
			...
			for (ImageSource<T> source : sources) {
				/*
				 * if the image is from a BufferedImage, then we require that
				 * the output format be set. (or else, we can't tell what format
				 * to output as!)
				 */
				if (source instanceof BufferedImageSource) {
					if (isOutputFormatNotSet()) {
