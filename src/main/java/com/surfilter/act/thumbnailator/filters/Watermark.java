...
public class Watermark implements ImageFilter {
	 ...
	 */
	public Watermark(Position position, BufferedImage watermarkImg, float opacity, Integer degree) {
		if (position == null) {
	...
	public BufferedImage apply(BufferedImage img) {
		...
		BufferedImage waterImg = watermarkImg;
		// 4、设置水印旋转
		if (null != degree) {
			try {
				waterImg = FontImage.rotateImage(degree, waterImg);
			} catch (IOException e) {
			}
		}

		int watermarkWidth = waterImg.getWidth();
		int watermarkHeight = waterImg.getHeight();
		...
		g.drawImage(waterImg, p.x, p.y, null);
